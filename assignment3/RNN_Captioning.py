...
for i, (caption, url) in enumerate(zip(captions, urls)):
    ...
    plt.savefig('./reports/sample_images_{}.png'.format(i))
    plt.clf()
...
# Run the following to check your implementation. You should see error around `1e-8`.
...
# Because this is very similar to the affine layer that you implemented in assignment 2,
# we have provided this function for you in the `temporal_affine_forward` and `temporal_affine_backward` functions in the file `cs231n/rnn_layers.py`.
# Run the following to perform numeric gradient checking on the implementation. You should see errors less than 1e-9.
...
# After doing so, run the following to check your forward pass using a small test case; you should see error less than `1e-10`.
...
# on this assignment we use a `CaptioningSolver` class to train image captioning models.
...
plt.savefig('./reports/overfit_image_captions.png')
...
# 
# In the file `cs231n/classifiers/rnn.py`, implement the `sample` method for test-time sampling. After doing so, run the following to sample from your overfitted model on both training and validation data.
...
for split in ['train', 'val']:
    ...
    for gt_caption, sample_caption, url in zip(gt_captions, sample_captions, urls):
        ...
        plt.axis('off')
        plt.savefig('./reports/image_captions_val_{}.png'.format(count))
        plt.clf()
        count = count + 1
